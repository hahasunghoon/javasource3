 ...
 * 
 * ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★ 외우기 면접에서도 물어봄
 * List 인터페에스 : 순서가 존재함, 데이터 중복을 허용함 (배열생각하자)
 ...
 *                            순서를 유지하지않음. key는 중복안되고 value(값)은 중복 허용 
...
public class ListEx1 {
	...
	public static void main(String[] args) {
		...
		ArrayList<String> list2 = new ArrayList<>(); //문자열
		
		System.out.println("size() "+list.size()); // 배열의 length 와 같은 개념
		System.out.println("isEmpty() "+list.isEmpty()); //자주쓰임
		...
		list2.add("자바"); //2.add(string)
		list2.add("html"); //2.add(string)
		list2.add("script"); //2.add(string)
		list2.add("서블릿"); //2.add(string)
		...
		//배열 =>리스트로 변환 (추가할 내용이 없다면)
		 ...
		 //add(int index, 요소) add란 추가란의미 ,index란 3에 6이라는 숫자를 넣겠다는 의미 
		 list.add(3, 6);
		 System.out.println("특정 위치에 추가 후 "+list);
		 
		 //set(int index, 변경할 요소) 바꾸는것 
		 list.set(3, 7);
		...
		//remove(Object o)
		 ...
		 //Collections.sort() : 오름차순 기본, 내림차순을 원할 떄는 기준 제공
