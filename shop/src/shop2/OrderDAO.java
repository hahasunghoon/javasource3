...
//JdbcUtil 클래스 : 드라이버로드, connection, close 반복잡업을 하나의 클래스에 정의
...
public class OrderDAO {
	...
	// 장바구니 구현 == > sorder 추가
	public boolean insert(int productId) {
	...
	public List<OrderDTO> carts(int userId) {
		...
		try {
			...
			String sql = "SELECT s.user_id, u.name, t.info, s.product_id, p.pname, p.price, p.content, s.order_date "
					+ "FROM sorder s, suser u, paytype t, product "
			...
			// ResultSet 결과 : user_id, name, name, product_id, p.price, p.content = OrderDTO 담기
	...
	public void insert(int selUser, int parseInt) {
