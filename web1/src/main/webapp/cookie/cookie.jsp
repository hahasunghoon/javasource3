...
<body>
	<%-- 
   세션과 쿠키
   세션 : 서버쪽에 저장 (객체 상태로 저장가능)
   쿠키 : 클라이언트 쪽에 저장 (객체로 저장 불가)
         만료기간 지정, 저장할 수 있는 양이 정해져 있음
   
   뭽 스토리지 : 브라우저에 저장(스크립트에서 할 수 있음)
   ...
   =>
   cookie1.jsp 응답(브라우저 헤더 정보에 쿠키 정보를 같이보냄)
   =>
   클라이언트 컴퓨터에 쿠키 정보가 저장됨
   =>
   http://localhost:8080/cookie/getCookie1.jsp 요청
   =>
   쿠키 저장소에 있는 http://localhost:8080 이 저장해 놓은 값들이 따라가게됨
   =>
